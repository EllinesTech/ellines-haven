...
// ── Activate: wipe ALL old caches, claim all tabs, force reload ──────────────
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys()
      ...
      .then(() => {
        // Tell all open tabs to reload so they get the fresh JS/CSS
        return self.clients.matchAll({ type: 'window' }).then(clients => {
          clients.forEach(client => {
            // Only reload non-reader pages so reading progress isn't lost
            if (!client.url.includes('/read/')) {
