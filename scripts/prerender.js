...
function patchHtml(template, route) {
  ...
  // 7. Crawlable body OUTSIDE #root — hidden from users via inline style but
  //    readable by Googlebot. Inline style is self-contained per element so it
  //    can never be accidentally stripped. React mounts into #root (a sibling);
  //    it never touches or re-renders #eh-seo-crawl.
  const seoBody = buildSeoBody(route);
  const SEO_HIDE = 'position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0;clip-path:inset(50%)';
  const seoBlock = `<div id="eh-seo-crawl" data-eh-seo="1" style="${SEO_HIDE}">${seoBody}</div>`;
