...
import { collection, query, where, onSnapshot, doc, getDoc, getDocFromCache, setDoc, updateDoc, serverTimestamp } from 'firebase/firestore';
...
// -- Pending Order Row — Retry Activation via server verify only --------------
// Cloud Function confirms with Paystack AND unlocks. Client never grants
// library access from an unverified payment.
function PendingOrderRow({ order: o, userEmail, isPendingPaystack }) {
  ...
  const retryActivation = async () => {
    ...
    try {
      ...
      if (!o.paystackRef) {
        setRetryMsg('No Paystack reference on this order. Please contact support.');
        return;
      }
      ...
      try {
        ...
        if (/Payment status:/i.test(msg) && !/pending/i.test(msg)) {
...
export default function MyLibrary() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (library?.length) {
          const savedIds = new Set(list.map((b) => String(b.bookId)));
          const map = {};
          library.forEach((b) => { map[b.id] = savedIds.has(String(b.id)) ? 'saved' : false; });
          setOfflineState(map);
        }
      } catch {
  ...
  const TABS = [
    ...
    { k:'offline',     label:'Downloaded',  badge: offlineCount || null       },
                              ...
                              <>
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      let chapters = [];
                                      const ref = doc(db, 'book_chapters', String(b.id));
                                      try {
                                        const cached = await getDocFromCache(ref);
                                        if (cached.exists()) chapters = cached.data().chapters || [];
                                      } catch { /* cache miss */ }
                                      if (!chapters.length) {
