  ...
  return (
    <div className="lb-overlay" onClick={onClose} aria-modal="true" role="dialog" aria-label="Cover zoom">
      ...
      >
        ...
        >
          {hasImageCover(book)
            ? <CoverImage src={book.cover} alt={book.title} className="lb-img" draggable="false" priority />
...
function OngoingSeriesPurchase({ book, owned, libLoaded }) {
  ...
  return (
    <div id="bd-series-purchase" style={{
      ...
      {/* Free first chapter banner — guests and logged-in users */}
      {hasFreeSampleChapter(book) && !owned && (
        <div style={{ padding: '12px 18px', background: 'rgba(74,158,255,0.08)', borderBottom: '1px solid rgba(74,158,255,0.2)', textAlign: 'center' }}>
          <span style={{ fontSize: '0.8rem', color: '#4a9eff', fontWeight: 600 }}>🎁 Read the first chapter free — available to everyone</span>
          {' '}
          <Link to={readPath(book)} style={{ fontSize: '0.8rem', color: '#d4b5ff', fontWeight: 700, marginLeft: 8 }}>
            Start reading →
          </Link>
        </div>
      )}
...
export default function BookDetail() {
  ...
  // Show sticky bar once user scrolls past the purchase section
  useEffect(() => {
    const onScroll = () => {
      const purchaseEl = document.getElementById('bd-purchase-section');
      if (!purchaseEl) { setStickyVisible(false); return; }
  ...
  useEffect(() => {
    ...
    const bookImg  = hasImageCover(book) && book.cover.startsWith('/')
      ? `https://haven.ellines.co.ke${coverBasePath(book.cover)}`
      : hasImageCover(book) && /^https?:\/\//i.test(book.cover)
        ? book.cover
  ...
  return (
    <main className="bd-page">
                            ...
                            : siteControls?.readOnlyMode
                              ...
                              : user && myPerms?.canPurchase === false
                                ...
                                : !user
                                  ...
                                  : /* Logged-in — free sample still available before purchase */
      ...
      {/* ── Sticky buy bar — appears when price section scrolls out of view ── */}
      {stickyVisible && !owned && !NO_PURCHASE_STATUSES.has(book.status) && (
        <div className="bd-sticky-bar">
          <div className="bd-sticky-bar__inner">
            <div className="bd-sticky-bar__info">
              <strong className="bd-sticky-bar__title">{book.title}</strong>
              <span className="bd-sticky-bar__price">KSh {book.price}</span>
            </div>
            <div className="bd-sticky-bar__actions">
              {inCart ? (
                <Link to="/cart" className="btn btn-primary bd-sticky-bar__btn">Go to Cart →</Link>
              ) : user ? (
                <button
                  className="btn btn-primary bd-sticky-bar__btn"
              ...
              <a
                ...
                className="btn btn-wa bd-sticky-bar__wa"
